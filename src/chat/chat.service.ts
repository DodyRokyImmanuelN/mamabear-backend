...
const SAFETY_KEYWORDS = ['tidak untuk', 'catatan', 'peringatan', 'alergi', 'kontraindikasi', 'efek samping', 'dilarang', 'hindari'];
...
function summarizeDescription(description: string, maxLength = 1500): string {
    ...
    const missingSafetyLines = safetyLines.filter(line => !mainText.includes(line));
...
export class ChatService {
    ...
    async generateReply(message: string): Promise<string> {
        ...
        const searchResult = await this.searchService.findProductsBySemanticSearch({ q: message });
        const products = searchResult.data as any[];

        const productContext = products
            .map(p => `- ${p.name} (Rp${p.harga}): ${summarizeDescription(p.deskripsi)}`)
            .join('\n');


        const systemPrompt = `Kamu adalah asisten MamaBear, toko produk untuk ibu hamil dan menyusui. Jawab pertanyaan user HANYA berdasarkan produk di bawah ini. Jangan menyebut produk lain di luar daftar ini. Kalau tidak ada yang benar-benar cocok, katakan terus terang tidak ada, jangan memaksakan rekomendasi.\n\nPRODUK YANG RELEVAN:\n${productContext}`;
